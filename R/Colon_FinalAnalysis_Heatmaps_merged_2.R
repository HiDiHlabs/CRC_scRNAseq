########################################################################################################
########################## X. Heatmaps of curated factors across all cells ###################################
########################################################################################################
...
MergedFactors_GeneLists <- list()
for (m in 1:8){
  for (f in 1:length(MergedFactors[[m]])){
    if (f==1){
      MergedFactors_GeneLists[[m]] <- as.character(factors[,MergedFactors[[m]][f]])
    } else {
      MergedFactors_GeneLists[[m]] <- c(MergedFactors_GeneLists[[m]], as.character(factors[,MergedFactors[[m]][f]]))
    }
    MergedFactors_GeneLists[[m]] <- unique(MergedFactors_GeneLists[[m]])  
  }
}
...
for (c in 1:length(colNames)){
  for (m in 1:NrMergedFactors){
    CellScoresMatrix[m,c] <- mean(counts[MergedFactors_GeneLists[[m]],c])
  }
}
